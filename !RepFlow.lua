...
local colorCode = imgui.new.char[8](colorCode or "{1E90FF}") -- Значение по умолчанию
local tag = colorCode[0] .. "[RepFlow]: {FFFFFF}"
local taginf = colorCode[0] .. "[Информация]: {FFFFFF}"
...
local function updateTags()
    ...
    taginf = colorCode[0] .. "[Информация]: {FFFFFF}"
end
...
function drawSettingsTab()
    ...
    -- Поле для изменения colorCode
    if imgui.InputText("Цветовой код", colorCode, ffi.sizeof(colorCode)) then
        updateTags() -- Обновляем переменные при изменении значения
    end
    
    -- Кнопка для сохранения изменений в цвете
    if imgui.Button(u8"Сохранить цвет") then
        saveColorCode() -- Сохраняем новый цвет в конфиг
        sampAddChatMessage(taginf .. "Цвет тегов обновлён на: " .. colorCode[0], -1)
    end

    imgui.Text(u8"Текущий тег: " .. tag)
    imgui.Text(u8"Текущий тег информации: " .. taginf)
end

function saveColorCode()
    ini.main.colorCode = colorCode -- сохраняем новый цвет
    inicfg.save(ini, IniFilename)
end
